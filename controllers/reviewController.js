...
const User = require('../models/User.js'); // Not strictly necessary, but good to keep if referenced

/**
 * @route   POST /api/reviews/:bookingId
 * @desc    Create a new review for a service via a completed booking
 * @access  Private (User)
 * @MODIFIED: Populate service with workshop for context.
 */
const createServiceReview = async (req, res) => { // MODIFIED: Use exports.createServiceReview
    ...
    try {
        const booking = await Booking.findById(bookingId)
            .populate('customer')
            .populate({
                path: 'service', // Populate the service linked to the booking
                populate: {
                    path: 'workshop', // And then populate the workshop within that service
                    select: 'workshopName' // Select just the workshop name
                }
            });
        ...
        const review = {
            user: req.user.id,
            username: booking.customer?.fullName || 'Anonymous', // Use 'Anonymous' if fullname is missing
...
// --- MODIFIED: Use module.exports for consistency ---
module.exports = {
    createServiceReview
};