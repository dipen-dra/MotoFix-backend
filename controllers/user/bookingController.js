...
const awardLoyaltyPoints = async (userId) => {
    ...
    if (user) {
        const pointsToAdd = Math.floor(Math.random() * 11) + 10; // Random points between 10 and 50
...
const updateUserBooking = async (req, res) => {
    try {
        ...
        if (booking.status !== 'Pending') return res.status(400).json({ success: false, message: `Cannot edit a booking with status "${booking.status}"` });
